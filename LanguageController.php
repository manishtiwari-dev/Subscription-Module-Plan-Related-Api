...
class LanguageController extends Controller
{
    ...
    public function trans_edit_key_value(Request $request)
    {
        ...
        // return ApiHelper::JSON_RESPONSE(true,$all_key_group,'');
        ...
        if (!empty($all_key_group)) {
            ...
            foreach ($all_key_group as $key => $group) {
                ...
                $all_key_group_value_pair[$group->section_id]['all_trans_key_val_pair'] = $all_trans_key_val_pair;


                if ($group->section_id == 0)
                    $all_key_group_value_pair[$group->section_id]['group_name'] = "Default";
                else
                    $all_key_group_value_pair[$group->section_id]['group_name'] = !empty($section) ? $section->section_name : '';
            }
        }
