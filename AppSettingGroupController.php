...
class AppSettingGroupController extends Controller
{
    ...
    public function update(Request $request)
    {
        ...
        if ($validator->fails())
            return ApiHelper::JSON_RESPONSE(false, [], $validator->messages());
        $setting_update_data = $request->only(['group_name', 'access_privilege', 'status', 'sort_order']);
        AppSettingsGroup::where('group_id', $request->group_id)->update($setting_update_data);

        if ($setting_update_data)
            return ApiHelper::JSON_RESPONSE(true, $setting_update_data, 'SUCCESS_SETTING_GROUP_UPDATE');
        else
